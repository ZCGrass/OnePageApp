  ...
  constructor(props) {
    ...
    this.columns[this.columns.length - 1].render = (text, record) => (
      <div className="table-action">
        <Button size="small" onClick={() => this.handleSetPermissions(text, record)}><FormattedMessage id="da_Authority" /></Button>
      </div>
    );
    message.config({
      top: 100,
    ...
    post(this.state.getUrl, value, (res) => {
      if (res.status === 1) {
        this.setState({
          ...
          treeNodes: res.data.dimensions.treeNodes,
          loading: false,
        });
      }
    });
    ...
    return (
      <div className="common-table">
        <Row>
          ...
          <Col span={8}>
            <Search
              placeholder={this.props.intl.formatMessage({ id: 'da_searchPlaceHolder' })}
              size="large"
